    ...
    <div ref={containerRef} className="relative">
      <input
        ...
        className={`w-[40rem] below-695:w-[30rem] below-566:w-[20rem] 
          below-500:w-[15rem] below-500:text-[0.8rem] outline-none bg-white text-black p-2 border-2 border-black   flex self-center justify-center border-b-8 border-r-4 `}
      ...
      />
      {isDropdownVisible && (
        <div className="absolute bg-white border border-black w-[40rem] mt-2 z-20 max-h-[30rem]  below-500:w-[15rem]  overflow-y-auto ">
          {inputValue.trim() === "" ? (
            recentSearches.length > 0 ? (
              <>
                <div className="p-2 text-red-500 font-bold below-500:text-[1rem]">
                  Recent Searches
                </div>
                {recentSearches.map((search, index) => (
                  <div
                    key={index}
                    className="p-2 cursor-pointer hover:bg-gray-200 text-red-500 below-500:text-[0.8rem]"
                    onClick={() =>
              ...
              <div className="p-2 text-gray-500 below-500:text-[0.8rem]">No recent searches</div>
            )
          ) : suggestions.length > 0 ? (
            suggestions.map((suggestion, index) => (
              <div
                key={index}
                className="p-2 cursor-pointer hover:bg-gray-200 below-500:text-[0.8rem] "
                onClick={() =>
                  handleSuggestionClick({
            ...
            <div className="p-2 text-gray-500 below-500:text-[0.8rem] ">No products found</div> // Show message when no suggestions
          )}
        </div>
      )}
    </div>
